...
# Ouverture d'un fichier reprenant les choix du paramètre par la méthode grid search
f4 = open('gs_param.txt','w')
...
for t in range(10):
    ...
    # Sélection des paramètres et des valeurs à tester pour le grid search
    parametres = {'max_depth' : [3,4,5,8,10,20,30]}

    # Modèle de classification
    clf = ExtraTreesClassifier()

    # Grid Search
    gs = GridSearchCV(clf,parametres)
    ...
    for i in range(0,len(X)-1,4):
        ...
        # Ecriture du choix du paramètre
        f4.write('%d\n' %gs.best_params_["max_depth"])
        ...
        # Calcul du log loss
        y_pred_probs = gs.predict_proba(X_test)
